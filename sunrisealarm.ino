...
#define PIXEL_PIN D2
#define PIXEL_TYPE WS2812B_FAST
#define PIXEL_COUNT 60 // 232
...
unsigned int colorValues = 210; // 210

// define sunrise duration (times #colorValues)
unsigned int duration = 4300; // 8600
...
void setup() {
    ...
    Particle.function("off", cloudOff);
...
int cloudHandleWeekdayParams(String command) {
    ...
    String days = String(command.substring(0, 5));
    String hour = String(command.substring(5, 7));
    String min  = String(command.substring(7));
    
    String s_hour, s_min;
    long i_hour, i_min;

    if (hour.startsWith("0")) {
        s_hour = hour.substring(1,2);
    } else {
        s_hour = hour;
    }
    char charBuf_hour[2];
    s_hour.toCharArray(charBuf_hour, 2);
    i_hour = atoi(charBuf_hour);
    
    if (min.startsWith("0")) { 
        s_min = min.substring(1,2);
    } else {
        s_min = min;
    }
    char charBuf_min[2];
    s_min.toCharArray(charBuf_min, 2);
    i_min = atoi(charBuf_min);
    
    debug.concat(String(s_hour));
    debug.concat(String(':'));
    debug.concat(String(s_min));
