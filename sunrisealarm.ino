...
void setup() {
    Time.zone(+2); // Amsterdam, Paris, Berlin, ...

    // create the alarms 
    Alarm.alarmRepeat(dowMonday, 6,0,0, MorningAlarm);
    Alarm.alarmRepeat(dowTuesday, 5,55,0, MorningAlarm);
    Alarm.alarmRepeat(dowWednesday, 5,50,0, MorningAlarm);
    Alarm.alarmRepeat(dowThursday, 5,45,0, MorningAlarm);
    Alarm.alarmRepeat(dowFriday, 5,40,0, MorningAlarm);
    Alarm.alarmRepeat(7,15,0, MorningOffAlarm);
...
void MorningAlarm() {
    // Particle.publish("MorningAlarm triggered");
    sunrise();
};

void MorningOffAlarm() {
    // Particle.publish("MorningOffAlarm triggered");
    pixelsOff();
};